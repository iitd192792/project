...
def extract_features(executables_dir,feature_version=2):
    if feature_version ==2:
        data  = np.empty((0,2381))
    else:
        data  = np.empty((0,2351))
            
    files = [f for f in os.listdir(executables_dir)]
    for file in files:
        binary_path = os.path.join(executables_dir,file )
        if not os.path.exists(binary_path):
            print("{} does not exist".format(binary_path))
        else:
            file_data = open(binary_path, "rb").read()
            features = PEFeatureExtractor(feature_version=feature_version,print_feature_warning=False).feature_vector(file_data)
